...
public class RoleSpec implements Specification<RoleEntity>{
    ...
    CriteriaBuilder cb){
        ...
        if (roleModel.getActorId() != null && roleModel.getId() != ""){
            p.getExpressions().add(cb.equal(root.get("id"), roleModel.getId()));
        }
        ...
        .equalsIgnoreCase(GlobalConstant.REC_STATUS_ACTIVE) || roleModel.getRecStatus().trim()
        .equalsIgnoreCase(GlobalConstant.REC_STATUS_NON_ACTIVE))){
            p.getExpressions().add(cb.equal(cb.upper(root.get("recStatus")), roleModel.getRecStatus().toUpperCase()));
        }
